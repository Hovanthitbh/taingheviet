...
<?php
	...
	if(!isset($_GET['pr'])){
		$result = mysqli_query($conn, "select count(ProductID) as total from product where CategoryID = 1");
		...
		if(!isset($_GET['sapxep'])){
			$query_page = "SELECT * FROM product WHERE CategoryID = 1 LIMIT $start, $limit";	
		}
		if(isset($_GET['sapxep'])){
			$sx= $_GET['sapxep'];
			if($sx == 1){
				$query_page = "SELECT * FROM product WHERE CategoryID = 1 ORDER  BY UnitPrice DESC LIMIT $start, $limit";	
			}
			if ($sx==2) {
				$query_page = "SELECT * FROM product WHERE CategoryID = 1 ORDER  BY UnitPrice LIMIT $start, $limit";	
			}
			if ($sx==3) {
				$query_page = "SELECT * FROM product WHERE CategoryID = 1 LIMIT $start, $limit";
			}
...
<body>
	<div id="display">
		<div>
			<form action="display_product_headphone.php" method="get">
		...
		<div id="phantrang" style="width: 500px; height: 50px; margin-top: 20px; clear: left;" align="center">
			<?php
				if(!isset($_GET['pr']) && !isset($_GET['sapxep'])){ ?>
					...
					<?php if($curent_page>1 && $total_page>1){?>
						<li><a href="display_product_headphone.php?page=<?=($curent_page - 1) ?>">&lt;</a><li>
					<?php }
					for($i =1; $i<=$total_page;$i++){
						if($i== $curent_page)
							echo "<li class='active'><a href='display_product_headphone.php?page=".$i."'>".$i."</a><li>"; 
						else { ?><li><a href="display_product_headphone.php?page=<?= $i ?>"><?= $i ?></a><li>
					<?php }}
					...
					if ($sx == 1) { ?>
						<div class="text-center">
			                <div class="site-block-27">
			                  <ul>
						<?php if($curent_page>1 && $total_page>1){?>
							<li><a href="display_product_headphone.php?sapxep=1&page=<?=($curent_page - 1) ?>">&lt;</a></li>
						<?php }
						for($i =1; $i<=$total_page;$i++){
							if($i== $curent_page)
								echo "<li class='active'><a href='display_product_headphone.php?page=".$i."'>".$i."</a><li>"; 
							else { ?><li><a href="display_product_headphone.php?sapxep=1&page=<?= $i ?>"><?= $i ?></a></li>
						<?php }}
						if($curent_page<$total_page && $total_page>1){ ?>
								<li><a href="display_product_headphone.php?sapxep=1&page=<?= ($curent_page + 1) ?>">&gt;</a></li>
								</ul>
					...
					if ($sx == 2) { ?>
						<div class="text-center">
			                <div class="site-block-27">
			                  <ul>
						<?php if($curent_page>1 && $total_page>1){?>
							<li><a href="display_product_headphone.php?sapxep=2&page=<?=($curent_page - 1) ?>">&lt;</a></li>
						<?php }
						for($i =1; $i<=$total_page;$i++){
							if($i== $curent_page)
								echo "<li class='active'><a href='display_product_headphone.php?page=".$i."'>".$i."</a><li>"; 
							else { ?><li><a href="display_product_headphone.php?sapxep=2&page=<?= $i ?>"><?= $i ?></a></li>
						<?php }}
						if($curent_page<$total_page && $total_page>1){ ?>
								<li><a href="display_product_headphone.php?sapxep=2&page=<?= ($curent_page + 1) ?>">&gt;</a></li>
								</ul>
