...
<?php 
	...
	function adddotstring($strNum) {
		...
		$result = mysqli_query($conn, "select count(ProductID) as total from product where Promotion>0");
		...
		$query_page = "SELECT * FROM product WHERE Promotion > 0 LIMIT $start, $limit";	
 ?>	
 	<div id="display_product" style="min-height: 500px;margin-top: 50px;">
 		<?php
								...
								<?php 
									if($row['Quantity']>0){ ?>
										...
										<?php if ($row['Promotion']>0) : ?>
												<span class="btn btn-warning" style="font-size: 15px; height: 30px;">-<?= $row['Promotion'] ?> %</span>
												<span style="font-size: 15px;text-decoration: line-through; color: gray; ma">
        ...
        <div class="site-block-27">
          <ul>
			<?php if($curent_page>1 && $total_page>1){?>
				<li><a href="display_promotion.php?page=<?=($curent_page - 1) ?>">&lt;</a><li>
			<?php }
			for($i =1; $i<=$total_page;$i++){
				if($i== $curent_page)
					echo "<li class='active'><a href='display_promotion.php?page=".$i."'>".$i."</a><li>"; 
				else { ?><li><a href="display_promotion.php?page=<?= $i ?>"><?= $i ?></a><li>
			<?php }}
